...
async function bootstrapLocalSchema(db: PgliteDatabase<typeof schema>) {
  ...
  `));
  await db.execute(
    sql.raw(`CREATE UNIQUE INDEX IF NOT EXISTS "meal_groups_active_default_label_key" ON "meal_groups" USING btree ("user_id","label") WHERE "deleted_at" IS NULL AND "is_default" = true`),
  );
  await db.execute(sql.raw(`
